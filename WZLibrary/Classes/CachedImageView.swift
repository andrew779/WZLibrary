 ...
 */
open class WZCachedImageView: UIImageView {
    
    open static let imageCache = NSCache<NSString, WZDiscardableImageCacheItem>()
    ...
    open func loadImage(urlString: String, completion: (() -> ())? = nil) {
        ...
        if let cachedItem = WZCachedImageView.imageCache.object(forKey: urlKey) {
        ...
        URLSession.shared.dataTask(with: url, completionHandler: { [weak self] (data, response, error) in
            ...
            DispatchQueue.main.async {
                if let image = UIImage(data: data!) {
                    let cacheItem = WZDiscardableImageCacheItem(image: image)
                    WZCachedImageView.imageCache.setObject(cacheItem, forKey: urlKey)
